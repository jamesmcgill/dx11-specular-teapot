...
DX::DeviceResources::CreateDeviceResources()
{
  ...
  if (adapter)
  {
    hr = D3D11CreateDevice(
      ...
      D3D11_SDK_VERSION,
      m_d3dDevice
        .ReleaseAndGetAddressOf(),    // Returns the Direct3D device created.
...
void
DX::DeviceResources::SetWindow(HWND window, int width, int height)
{
  m_window = window;
...
bool
DX::DeviceResources::WindowSizeChanged(int width, int height)
{
  RECT newRc;
...
DX::DeviceResources::Present()
{
  ...
  if (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET)
  {
    ...
    sprintf_s(
      ...
      (hr == DXGI_ERROR_DEVICE_REMOVED) ? m_d3dDevice->GetDeviceRemovedReason()
                                        : hr);
    OutputDebugStringA(buff);
#endif
    HandleDeviceLost();
...
void
DX::DeviceResources::GetHardwareAdapter(IDXGIAdapter1** ppAdapter)
{
  *ppAdapter = nullptr;
